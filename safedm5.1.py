...
class BiliDanmakuRestorer:
    ...
    def __init__(self, root):
        self.root = root
        root.title("B站弹幕补档工具 正式版 v5.1")
        root.geometry("1100x900")
        self.color_format = tk.IntVar(value=0)
        self.xml_path = tk.StringVar()
        self.cid_list = []
        self.pages = []
        self.checkpoint_file = Path("dm_checkpoint.pkl")
    ...
    def create_widgets(self):
        ...
        ttk.Checkbutton(control_frame, text="断点续传", variable=self.resume_mode).pack(side="left", padx=10)
        ttk.Button(control_frame, text="清除记录", command=self.clean_checkpoint).pack(side="left", padx=10)
        ...
        self.auto_shutdown = ttk.Checkbutton(control_frame, text="自动关机", variable=self.auto_shutdown_choose)
    ...
    def clean_checkpoint(self):
        if self.checkpoint_file.exists():
            try:
                ...
                self.log(f"清除记录失败: {str(e)}")
        else:
            self.log("未找到可清除的进度记录")

    def save_checkpoint(self, danmaku_list, success_count):
        checkpoint_data = {
            'version': 2,
            'bvid': self.bvid_entry.get().strip(),
            'cid': self.cid_list[self.part_combobox.current()],
        ...
        try:
            with open(self.checkpoint_file, 'rb') as f:
                ...
                if data.get('version', 1) != 2:
                    raise ValueError("旧版本记录格式")
                if data['bvid'] != self.bvid_entry.get().strip():
                    raise ValueError("BV号不匹配")
                if data['cid'] != self.cid_list[self.part_combobox.current()]:
                    raise ValueError("分P不匹配")
                return data
        except Exception as e:
            self.log(f"进度加载失败: {str(e)}")
            return None
    ...
    def restore_process(self):
        try:
            checkpoint = self.load_checkpoint() if self.resume_mode.get() else None
            ...
            if checkpoint:
                if len(danmaku_list) != checkpoint['total']:
                    self.log("弹幕文件已修改，无法续传")
                    return
                
                self.current_index = checkpoint['current_index']
                success = checkpoint['success_count']
                self.log(f"检测到进度记录，将从第 {self.current_index+1}/{len(danmaku_list)} 条继续")
            else:
                self.current_index = 0
                success = 0
            ...
            with requests.Session() as session:
                ...
                for idx in range(self.current_index, total):
                    if self.stop_event.is_set():
                        self.save_checkpoint(danmaku_list, success)
                        break
            ...
            self.log(f"完成：成功发送 {success}/{total} 条弹幕")
